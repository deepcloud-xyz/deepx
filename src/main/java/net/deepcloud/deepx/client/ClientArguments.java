...
import net.deepcloud.deepx.common.JobPriority;
...
class ClientArguments {
  ...
  Properties inputs;
  Properties outputs;
  String deepxCacheFiles;
  String deepxCacheArchives;
  int priority;
  String queue;
  String userPath;
  ...
  private void init() {
    appName = "";
    appType = DeepXConfiguration.DEFAULT_DEEPX_APP_TYPE.toUpperCase();
    amMem = DeepXConfiguration.DEFAULT_DEEPX_AM_MEMORY;
    amCores = DeepXConfiguration.DEFAULT_DEEPX_AM_CORES;
    workerMemory = DeepXConfiguration.DEFAULT_DEEPX_WORKER_MEMORY;
    workerVCores = DeepXConfiguration.DEFAULT_DEEPX_WORKER_VCORES;
    workerNum = DeepXConfiguration.DEFAULT_DEEPX_WORKER_NUM;
    chiefWorkerMemory = workerMemory;
    evaluatorWorkerMemory = workerMemory;
    psMemory = DeepXConfiguration.DEFAULT_DEEPX_PS_MEMORY;
    psVCores = DeepXConfiguration.DEFAULT_DEEPX_PS_VCORES;
    psNum = DeepXConfiguration.DEFAULT_DEEPX_PS_NUM;
    deepxFiles = null;
    libJars = null;
    launchCmd = "";
    deepxCacheFiles = "";
    ...
    dockerImage = "";
    containerType = "";
    priority = DeepXConfiguration.DEFAULT_DEEPX_APP_PRIORITY;
    queue = "";
    userClasspathFirst = DeepXConfiguration.DEFAULT_DEEPX_USER_CLASSPATH_FIRST;
    boardIndex = DeepXConfiguration.DEFAULT_DEEPX_TF_BOARD_WORKER_INDEX;
    boardReloadInterval = DeepXConfiguration.DEFAULT_DEEPX_TF_BOARD_RELOAD_INTERVAL;
    boardEnable = DeepXConfiguration.DEFAULT_DEEPX_TF_BOARD_ENABLE;
    boardLogDir = DeepXConfiguration.DEFAULT_DEEPX_TF_BOARD_LOG_DIR;
    boardHistoryDir = DeepXConfiguration.DEFAULT_DEEPX_TF_BOARD_HISTORY_DIR;
    boardModelPB = DeepXConfiguration.DEFAULT_DEEPX_BOARD_MODELPB;
    boardCacheTimeout = DeepXConfiguration.DEFAULT_DEEPX_BOARD_CACHE_TIMEOUT;
    isRenameInputFile = DeepXConfiguration.DEFAULT_DEEPX_INPUTFILE_RENAME;
    streamEpoch = DeepXConfiguration.DEFAULT_DEEPX_STREAM_EPOCH;
    inputStreamShuffle = DeepXConfiguration.DEFAULT_DEEPX_INPUT_STREAM_SHUFFLE;
    inputFormatClass = DeepXConfiguration.DEFAULT_DEEPX_INPUTF0RMAT_CLASS;
    outputFormatClass = DeepXConfiguration.DEFAULT_DEEPX_OUTPUTF0RMAT_CLASS;
    inputStrategy = DeepXConfiguration.DEFAULT_DEEPX_INPUT_STRATEGY.toUpperCase();
    outputStrategy = DeepXConfiguration.DEFAULT_DEEPX_OUTPUT_STRATEGY.toUpperCase();
    tfEvaluator = DeepXConfiguration.DEFAULT_DEEPX_TF_EVALUATOR;
    outputIndex = -1;
    ...
    allOptions.addOption("appType", "app-type", true,
        "set the Application type, default \"DEEPX\"");
    ...
    allOptions.addOption("files", "files", true,
        "Location of the DeepX files used in container");
    allOptions.addOption("jars", "jars", true,
        "Location of the DeepX lib jars used in container");

    allOptions.addOption("launchCmd", "launch-cmd", true, "Cmd for DeepX program");
    ...
    allOptions.addOption("cacheFile", "cacheFile", true,
        "add the DeepX hdfsFile PATH");
    allOptions.addOption("cacheArchive", "cacheArchive", true,
        "add the DeepX hdfsPackage PATH");
    ...
    OptionBuilder
        .withValueSeparator('=');
    OptionBuilder
        .withDescription("DeepX configure");
    ...
    OptionBuilder
        .withValueSeparator('#');
    OptionBuilder
        .withDescription("dfs location,representing the source data of DeepX");
    ...
    OptionBuilder
        .withValueSeparator('#');
    OptionBuilder
        .withDescription("dfs location,representing the DeepX result");
  ...
  private void cliParser(String[] args) throws ParseException, IOException, ClassNotFoundException {
    ...
    if (appName.trim().equals("")) {
      appName = "DeepX-" + System.currentTimeMillis();
    }
    ...
    if (cliParser.hasOption("conf")) {
      ...
      if (!"TENSORFLOW".equals(appType) && !"MXNET".equals(appType) && !appType.equals("LIGHTLDA") && !appType.equals("XFLOW")) {
        if (confs.containsKey("deepx.ps.num")) {
          confs.setProperty("deepx.ps.num", "0");
        }
      }
    }
    ...
    if (cliParser.hasOption("cacheFile")) {
      deepxCacheFiles = cliParser.getOptionValue("cacheFile");
    }

    if (cliParser.hasOption("cacheArchive")) {
      deepxCacheArchives = cliParser.getOptionValue("cacheArchive");
    }

    if (cliParser.hasOption("files")) {
      deepxFiles = StringUtils.split(cliParser.getOptionValue("files"), ",");
    }

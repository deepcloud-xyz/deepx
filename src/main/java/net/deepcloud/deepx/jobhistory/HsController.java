...
public class HsController extends Controller implements AMParams {
  ...
  public void job() throws IOException {
    DeepXConfiguration deepxConf = new DeepXConfiguration();
    jobLogPath = new Path(conf.get(DeepXConfiguration.DEEPX_HISTORY_LOG_DIR,
        DeepXConfiguration.DEFAULT_DEEPX_HISTORY_LOG_DIR) + "/" + $(APP_ID) + "/" + $(APP_ID));
    LOG.info("jobLogPath:" + jobLogPath);
    String line = null;
    try {
      FSDataInputStream in = jobLogPath.getFileSystem(deepxConf).open(jobLogPath);
      ...
      for (String info : readLog.keySet()) {
        if (info.equals(AMParams.APP_TYPE)) {
          if (readLog.get(info) != null) {
            if (readLog.get(info).equals("DEEPX")) {
              set(APP_TYPE, "DeepX");
            } else {
          ...
          if (readLog.get(info) instanceof ArrayList<?>) {
            ...
            if (outputList.size() == 0 || outputList.get(0).equals("-")) {
              ...
              for (String output : outputList) {
                set(OUTPUT_PATH + j, output + conf.get(DeepXConfiguration.DEEPX_INTERREAULST_DIR, DeepXConfiguration.DEFAULT_DEEPX_INTERRESULT_DIR));
                j++;
              }
          ...
          if (readLog.get(info) instanceof Map<?, ?>) {
            ...
            if (containerMessage.get(AMParams.CONTAINER_ROLE).equals(DeepXConstants.EVALUATOR) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(DeepXConstants.CHIEF)) {
              set(CONTAINER_ROLE + i, DeepXConstants.WORKER + "/" + containerMessage.get(AMParams.CONTAINER_ROLE));
            } else {
            ...
            if (containerMessage.containsKey(AMParams.CONTAINER_CPU_METRICS)) {
              ...
              if (cpuMetrics != null) {
                ...
                if (map.size() > 0) {
                  cpuMetricsFlag = true;
                  if (containerMessage.get(AMParams.CONTAINER_ROLE).equals(DeepXConstants.WORKER) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(DeepXConstants.EVALUATOR) || containerMessage.get(AMParams.CONTAINER_ROLE).equals(DeepXConstants.CHIEF)) {
